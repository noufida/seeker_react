  ...
  const getHandler=async()=>{
    ...
    {headers:{Authorization:`Bearer ${authTokens?.token}`}} ).then((response)=>{
       ...
       const addHandler=async()=>{
        console.log('kkk')
        await axios.post(`employer/location/`,{location:lo},
        {headers:{Authorization:`Bearer ${authTokens?.token}`}} ).then((response)=>{
            console.log(response.data,"application")
     ...
     const editHandler=async()=>{
        console.log('kkk')
        await axios.patch(`employer/location/${id}/`, {location:locan},
        {headers:{Authorization:`Bearer ${authTokens?.token}`}} ).then((response)=>{
            console.log(response.data,"application")
            if (response.status === 200) {
                console.log(response.data,"application")
     ...
     const deleteHandler=async(idd)=>{
        console.log('kkk')
        await axios.delete(`employer/location/${idd}/`, 
        {headers:{Authorization:`Bearer ${authTokens?.token}`}} ).then((response)=>{
            console.log(response.data,"application")
...
<Dialog open={open} onClose={handleClose}>
        ...
        <Row>
            <Col>
            <h1 className='m-5' > Job Locations</h1>
            </Col>
        </Row>
