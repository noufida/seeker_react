  ...
  const getHandler=async()=>{
    ...
    {headers:{Authorization:`Bearer ${authTokens?.token}`}} ).then((response)=>{
       ...
       const addHandler=async()=>{
        console.log('kkk')
        await axios.post(`razorpay/plans/`,
            {name:locan,amount:amount,valid_days:valid_days},
        {headers:{Authorization:`Bearer ${authTokens?.token}`}} ).then((response)=>{
            console.log(response.data,"application")
     ...
     const editHandler=async()=>{
        console.log('kkk')
        await axios.patch(`razorpay/plans/${id}/`, {name:locan,amount:amount,valid_days:valid_days},
        {headers:{Authorization:`Bearer ${authTokens?.token}`}} ).then((response)=>{
            console.log(response.data,"application")
            if (response.status === 200) {
                console.log(response.data,"application")
     ...
     const deleteHandler=async(idd)=>{
        console.log('kkk')
        await axios.delete(`razorpay/plans/${idd}/`, 
        {headers:{Authorization:`Bearer ${authTokens?.token}`}} ).then((response)=>{
            console.log(response.data,"application")
  ...
  return (
    <div>
        ...
        <TextField
            ...
            fullWidth
            variant="standard"
            onChange={(e)=>setValid_days(e.target.value)}
...
<Dialog open={open} onClose={handleClose}>
        ...
        <Row>
            <Col>
            <h1 className='m-5' > Employer Plans</h1>
            </Col>
        </Row>
